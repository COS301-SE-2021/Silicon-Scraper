...
module.exports = (dbase = db) => {
    ...
    const addCPUToWatchlist = async(userID, cpuID) => {
        const cs = new pg.helpers.ColumnSet(['user_id', 'product_id'], {table: 'watchlist_cpu'})
        const list = {
            user_id: userID,
            product_id: cpuID
        }
        const query = pg.helpers.insert(list, cs)
    ...
    const addGPUToWatchlist = async(userID, gpuID) => {
        const cs = new pg.helpers.ColumnSet(['user_id', 'product_id'], {table: 'watchlist_gpu'})
        const list = {
            user_id: userID,
            product_id: gpuID
        }
    ...
    const getWatchlist = async(userID) => {
        const where = pg.as.format('WHERE user_id = $1', userID);
        const cpuList: any[] = await dbase.manyOrNone("SELECT id, image, brand, model, price, description, availability, retailer, link, type FROM watchlist_cpu RIGHT JOIN cpus ON watchlist_cpu.product_id = cpus.id $1:raw", where)
        .then(cpus => {
            return cpus;
        })
        .catch(err => {
        ...
        const gpuList: any[] = await dbase.manyOrNone("SELECT id, image, brand, model, price, description, availability, retailer, link, type FROM watchlist_gpu RIGHT JOIN gpus ON watchlist_gpu.product_id = gpus.id $1:raw", where)
        .then(gpus => {
            return gpus;
        })
        .catch(err => {
    ...
    const removeProduct = async(userID, productID, productType) => {
        ...
        const where = pg.as.format('WHERE user_id = $1 AND product_id = $2', [userID, productID]);
