...
describe('WatchlistService test', () => {
    ...
    beforeEach(() => {
        ...
        repo.removeProductFromWatchlist = jest.fn((userID , productID, productType) => Promise.resolve(true))
    })

    // it('testing exception when parameters are invalid', async() => {
    //     expect(await watchlistRepo.addProduct({}, {})).toThrow(new InvalidRequestError())
    // })
    ...
    it('testing successful adding a CPU product', async() => {
        ...
        expect(repo.removeProductFromWatchlist.mock.calls.length).toBe(0)
    ...
    // it('testing unsuccessul retrival of users watchlist', async() => {
    //     let response = await watchlistService.getWatchlist({
    //         "id": 1
    //     })
    ...
    it('testing successful removal of product', async() => {
        ...
        expect(repo.removeProductFromWatchlist.mock.calls[0][0]).toBe(1)
        expect(repo.removeProductFromWatchlist.mock.calls[0][1]).toBe("ID")
        expect(repo.removeProductFromWatchlist.mock.calls[0][2]).toBe("CPU")
        expect(repo.removeProductFromWatchlist.mock.calls.length).toBe(1)
    })
})