  ...
  group("testing search service",(){
    ...
    group("applyFilters testing",(){
      test("get available should be 2",(){
        List<Product> p=applyFilters(list,true,false,false,false,0.0,0.000,false,false,false);
        expect(p.length, 2);
      });
      test("get limit stock should be 0",(){
        List<Product> p=applyFilters(list,false,true,false,false,0.0,0.0,false,false,false);
        expect(p.length, 0);
      });
      test("get out of stock should be 4",(){
        List<Product> p=applyFilters(list,false,false,true,false,0.0,0.0,false,false,false);
        expect(p.length, 4);
      });
      test("get not specified should be 1",(){
        List<Product> p=applyFilters(list,false,false,false,true,0.0,0.0,false,false,false);
        expect(p.length, 1);
      });
      test("get in range 0 to 1000000 should be 7",(){
        List<Product> p=applyFilters(list,false,false,false,false,0.0,1000000.0,false,false,false);
        expect(p.length, 7);
      });
      test("get in range 0 to 10000 should be 0",(){
        List<Product> p=applyFilters(list,false,false,false,false,0.0,10000.0,false,false,false);
        expect(p.length, 0);
      });
      test("get in range 0 to 10000 should be 7",(){
        List<Product> p=applyFilters(list,false,false,false,false,0.0,15000.0,false,false,false);
        expect(p.length, 1);
      });
      test("get relaiter evetech be 7",(){
        List<Product> p=applyFilters(list,false,false,false,false,0.0,0.0,true,false,false);
        expect(p.length, 7);
      });
      test("get relaiter other be 0",(){
        List<Product> p=applyFilters(list,false,false,false,false,0.0,0.0,false,true,false);
        expect(p.length, 0);
      });
    });
    });
